...
- (void)layoutSubviews
{
    ...
    if ([[self.postItem userHasLiked] boolValue]) {
...
- (void)setPostItem:(NSObject<InstaPost>*)postItem
{
    ...
    NSString *likeButtonTitle = postItem.userHasLiked.boolValue ? NSLocalizedString(@"Loved", nil) : NSLocalizedString(@"Love", nil);
    [self.likeButton setTitle:likeButtonTitle forState:UIControlStateNormal];
    
    if (postItem.userHasLiked.boolValue) {
